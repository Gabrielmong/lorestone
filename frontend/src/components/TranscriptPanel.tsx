...
export default function TranscriptPanel() {
  ...
  const displayed = showCleanOnly ? segments.filter((s) => s.isGameRelated) : segments
  ...
  const handleAssign = async (name: string) => {
    if (unknownSpeaker === null || !activeSessionId) return
    await recordingService.assignName(activeSessionId, unknownSpeaker, name)
    setAssigningName('')
  }

  const recDot = isRecording && (
    <Box sx={{
      width: 8, height: 8, borderRadius: '50%', bgcolor: '#b84848', flexShrink: 0,
  ...
  const langSelect = (
    <Box
      ...
      sx={{
        bgcolor: '#111009', border: '1px solid rgba(120,108,92,0.3)', color: '#786c5c',
        borderRadius: 0.5, px: 0.75, py: 0.25, fontSize: '0.72rem',
        cursor: isRecording ? 'not-allowed' : 'pointer', '&:focus': { outline: 'none' },
      }}
    >
  ...
  const recordBtn = (
    <Button
      ...
      sx={isRecording
        ? { bgcolor: '#b84848', '&:hover': { bgcolor: '#d45f5f' }, fontSize: '0.75rem', whiteSpace: 'nowrap' }
        : { borderColor: 'rgba(98,168,112,0.5)', color: '#62a870', '&:hover': { borderColor: '#62a870' }, fontSize: '0.75rem', whiteSpace: 'nowrap' }
      }
    >
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 1 }}>
      ...
      {isMobile ? (
        <>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            {recDot}
            <Typography sx={{ fontFamily: '"Cinzel"', fontSize: '0.75rem', color: '#786c5c', textTransform: 'uppercase', letterSpacing: 1, flex: 1 }}>
              Transcript
            </Typography>
            {langSelect}
            {filterBtn}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {isRecording && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, px: 1.5, py: 0.5, borderRadius: 1, bgcolor: 'rgba(184,72,72,0.12)', border: '1px solid rgba(184,72,72,0.4)' }}>
              {recDot}
              <Typography sx={{ fontSize: '0.75rem', color: '#b84848', fontFamily: '"JetBrains Mono"' }}>REC</Typography>
            </Box>
          )}
          {langSelect}
          {filterBtn}
      ...
      <Dialog
        open={unknownSpeaker !== null}
        onClose={() => {}}
        slotProps={{ paper: { sx: { bgcolor: '#1a1712', border: '1px solid rgba(120,108,92,0.3)' } } }}
      >
        ...
        <DialogActions>
          <Button
            size="small"
            onClick={() => {
              if (unknownSpeaker !== null) handleAssign(`Speaker ${unknownSpeaker}`)
            }}
            sx={{ color: '#786c5c' }}
          >
            Skip
          ...
          <Button
            size="small"
            variant="contained"
            disabled={!assigningName}
            onClick={() => { if (assigningName) handleAssign(assigningName) }}
  ...
  return (
    <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'flex-start' }}>
      <Chip
        ...
        size="small"
        sx={{
          bgcolor: 'transparent', border: `1px solid ${color}`, color,
          fontSize: '0.65rem', height: 20, flexShrink: 0, mt: 0.25,
        }}
      />
